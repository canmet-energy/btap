"""QUARANTINE FILE. Nothing here is NECB 2020/2025 geometry.

This is the legacy NECB 2011 daylighted-area math — the ports of
openstudio-standards' get_parameters_sidelighting /
get_parameters_skylight, tracking legacy behaviour statement for
statement. In the Ruby gem this file (daylighted_areas_legacy_2011.rb)
REOPENS ``Daylighting``, so every constant path is exactly what it always
was (``BtapNECB::Lighting::Daylighting.sidelighting_parameters`` /
``.skylight_parameters``); there are no delegators and no call site moved.
The FILENAME is the whole point: it labels the code as legacy so the
2020/2025 geometry in ``daylighted_areas.py`` is never confused with it.

PYTHON RE-EXPORT MECHANISM. Python cannot have two files contribute to one
module, so the reopening is a RE-EXPORT instead: ``daylighting.py`` does
``from btap.codes.necb.lighting._legacy_2011 import sidelighting_parameters,
skylight_parameters, costing_area_provider``, which keeps every caller's
constant path unchanged (``lighting.Daylighting.sidelighting_parameters``,
exactly as in Ruby) while the leading underscore and the file name keep the
quarantine visible. The two files are NOT merged — the quarantine IS the
point. Do not import this module directly from new code; go through
``daylighting`` if you must, and read the DO NOT BUILD ON IT note below.

PARITY-PINNED — MIRRORS LEGACY AS FIXED BY #2119 (merged 2026-07-15, in
tree since the origin/nrcan merge). ``test_daylighting_parity.rb`` diffs
these two methods against the legacy openstudio-standards implementation,
so their output must track it EXACTLY — which now means carrying #2119's
fix: the skylight-area accumulation happens ONCE PER SKYLIGHT, outside
the exterior-window loop, so a skylight-only space no longer computes
zero and multi-window spaces no longer double-count. The pre-#2119
defects are gone from BOTH sides.

What remains here are 2011-vs-2020 RULE differences, not defects:
per-window areas are SUMMED with no union (2011 had no "without
double-counting overlapping areas" language; 4.2.2.3.(1)/(5), 4.2.2.4.(1)
and 4.2.2.5.(1) do) and no secondary sidelighted area is computed (a 2020
concept). Do NOT "clean this up" or refactor it — any change must mirror
a change in legacy ``necb_2011.rb``, or the parity gate is meaningless.

DO NOT BUILD ON IT. Production callers are the legacy ``placement:
'necb2011'`` selection in ``daylighting.py`` and the legacy sensor-count rule
in btap.costing's ``lighting/fixtures.py``; new work wants
``daylighted_areas.py``, which implements the real NECB 2020/2025 Articles
4.2.2.3. (primary AND secondary sidelighted), 4.2.2.4. (roof monitors) and
4.2.2.5. (skylights) with unioned polygons. See L-26 and D-57.
"""

from __future__ import annotations

import math

from btap._compat import NullAudit, ruby_round, sorted_by_name

# ---- Daylighted-area geometry (ports of legacy, post-#2119,
# get_parameters_sidelighting / get_parameters_skylight) -------------------
#
# LEGACY-ONLY. These two methods exist to be diffed against legacy (as
# fixed by #2119) by test_daylighting_parity.rb; the 2020/2025 rule uses
# DaylightedAreas instead, because these SUM per-window areas with no
# union and so double-count overlaps, contrary to 4.2.2.3.(1)/(5),
# 4.2.2.4.(1) and 4.2.2.5.(1) ("the combined ... areas without
# double-counting overlapping areas"), and they compute no secondary
# sidelighted area at all — 2011-vs-2020 RULE differences, not defects.


def sidelighting_parameters(space, audit=None):
    """Legacy "primary sidelighted area" (its own comment cites NECB 2011
    4.2.2.9., an article that does not exist in 2020/2025): per exterior-wall
    window at floor level, depth = min(window head height, space depth), width
    = window width + min(distance to each side wall, 0.6 m). NECB 2020
    4.2.2.3.(3)-(4) instead specify HALF the window head height on each side,
    bounded by the distance to any vertical obstruction >= 1.5 m high.

    :return: {'area_m2':, 'vt_handle':, 'window_area_m2':} (vt_handle =
        sum(window area x VT), for the legacy 2011 effective aperture)
    """
    audit = audit if audit is not None else NullAudit()
    floor_surface = None
    floor_area = 0.0
    floor_vertices = []
    for surface in sorted_by_name(space.surfaces()):
        if surface.surfaceType() != 'Floor':
            continue

        floor_surface = surface
        floor_area += surface.netArea()
        floor_vertices.append(list(surface.vertices()))
    if floor_surface is None:
        return {'area_m2': 0.0, 'vt_handle': 0.0, 'window_area_m2': 0.0}

    area = 0.0
    vt_handle = 0.0
    window_area_sum = 0.0
    for surface in sorted_by_name(space.surfaces()):
        if not (surface.outsideBoundaryCondition() == 'Outdoors'
                and surface.surfaceType() == 'Wall'):
            continue

        try:
            surface_z_min = min(v.z() for v in surface.vertices())
            if surface_z_min != floor_vertices[0][0].z():
                continue

            wall_x = []
            wall_y = []
            for vertex in surface.vertices():
                if vertex.z() != surface_z_min:
                    continue

                wall_x.append(vertex.x())
                wall_y.append(vertex.y())
            if len(wall_x) < 2:
                continue

            opposite_x = []
            opposite_y = []
            for vertex in floor_vertices[0]:
                if ((vertex.x() != wall_x[0] and vertex.y() != wall_y[0])
                        or (vertex.x() != wall_x[1] and vertex.y() != wall_y[1])):
                    opposite_x.append(vertex.x())
                    opposite_y.append(vertex.y())
            width_wall = math.sqrt((wall_x[0] - wall_x[1]) ** 2 + (wall_y[0] - wall_y[1]) ** 2)
            width_opposite = math.sqrt((opposite_x[0] - opposite_x[1]) ** 2
                                       + (opposite_y[0] - opposite_y[1]) ** 2)
            depth = 2 * floor_area / (width_wall + width_opposite)

            for sub in sorted_by_name(surface.subSurfaces()):
                if sub.subSurfaceType() not in ('FixedWindow', 'OperableWindow'):
                    continue

                vt = sub.visibleTransmittance().get()
                window_area = sub.netArea()
                window_area_sum += window_area
                vt_handle += window_area * vt
                v = list(sub.vertices())
                if ruby_round(v[0].z(), 2) == ruby_round(v[1].z(), 2):
                    window_width = math.sqrt((v[0].x() - v[1].x()) ** 2
                                             + (v[0].y() - v[1].y()) ** 2)
                else:
                    window_width = math.sqrt((v[1].x() - v[2].x()) ** 2
                                             + (v[1].y() - v[2].y()) ** 2)
                head_height = ruby_round(max(vertex.z() for vertex in v), 2)
                area_depth = min(head_height, depth)
                plane = floor_surface.plane()
                projected = [plane.project(vertex) for vertex in v]
                side1 = min(math.sqrt((wall_x[0] - projected[0].x()) ** 2
                                      + (wall_y[0] - projected[0].y()) ** 2),
                            math.sqrt((wall_x[0] - projected[2].x()) ** 2
                                      + (wall_y[0] - projected[2].y()) ** 2), 0.6)
                side2 = min(math.sqrt((wall_x[1] - projected[0].x()) ** 2
                                      + (wall_y[1] - projected[0].y()) ** 2),
                            math.sqrt((wall_x[1] - projected[2].x()) ** 2
                                      + (wall_y[1] - projected[2].y()) ** 2), 0.6)
                area += area_depth * (side1 + window_width + side2)
        except Exception as e:
            audit.warn('daylighting',
                       f"sidelighting geometry failed on {surface.nameString()} "
                       f"({type(e).__name__}) — surface skipped")
    return {'area_m2': area, 'vt_handle': vt_handle, 'window_area_m2': window_area_sum}


def skylight_parameters(space, audit=None):
    """Legacy "daylighted area under skylights" + VT sums for the legacy 2011
    skylight effective aperture (its comment cites 4.2.2.7., which does not
    exist in 2020/2025). Mirrors legacy AS FIXED BY #2119: the area is
    accumulated once per skylight, AFTER the exterior-wall/window
    re-calculation loops, so a space with skylights but no exterior windows
    gets a real area and multi-window spaces do not double-count."""
    audit = audit if audit is not None else NullAudit()
    area = 0.0
    vt_handle = 0.0
    skylight_area_sum = 0.0
    roof_vertices = None
    for surface in sorted_by_name(space.surfaces()):
        if (surface.outsideBoundaryCondition() == 'Outdoors'
                and surface.surfaceType() == 'RoofCeiling'):
            roof_vertices = list(surface.vertices())

        for sub in sorted_by_name(surface.subSurfaces()):
            if sub.subSurfaceType() != 'Skylight':
                continue

            try:
                vt = sub.visibleTransmittance().get()
                s = list(sub.vertices())
                skylight_area_sum += sub.netArea()
                vt_handle += sub.netArea() * vt
                skylight_width = math.sqrt((s[0].x() - s[1].x()) ** 2 + (s[0].y() - s[1].y()) ** 2)
                skylight_length = math.sqrt((s[0].x() - s[3].x()) ** 2 + (s[0].y() - s[3].y()) ** 2)
                ceiling_height = s[0].z()
                r = roof_vertices
                if r is None:
                    continue

                lengths = [dist(r[0], r[1]), dist(r[1], r[2]), dist(r[2], r[3]), dist(r[3], r[0])]
                closest0 = min(s, key=lambda p: dist(r[0], p))
                closest2 = min(s, key=lambda p: dist(r[2], p))
                d1 = triangle_height(closest0, r[0], r[1], lengths[0])
                d2 = triangle_height(closest0, r[0], r[3], lengths[3])
                d3 = triangle_height(closest2, r[2], r[1], lengths[1])
                d4 = triangle_height(closest2, r[2], r[3], lengths[2])

                width = skylight_width + min(0.7 * ceiling_height, d1) + min(0.7 * ceiling_height, d4)
                length = skylight_length + min(0.7 * ceiling_height, d2) + min(0.7 * ceiling_height, d3)

                for wall in sorted_by_name(space.surfaces()):
                    if not (wall.outsideBoundaryCondition() == 'Outdoors'
                            and wall.surfaceType() == 'Wall'):
                        continue

                    w = list(wall.vertices())
                    wall_x = []
                    wall_y = []
                    if w[0].z() == w[1].z():
                        wall_x = [w[0].x(), w[1].x()]
                        wall_y = [w[0].y(), w[1].y()]
                    elif w[0].z() == w[3].z():
                        wall_x = [w[0].x(), w[3].x()]
                        wall_y = [w[0].y(), w[3].y()]
                    if len(wall_x) < 2:
                        continue

                    head_height = ruby_round(max(p.z() for p in s), 2)
                    wall_length = math.sqrt((wall_x[0] - wall_x[1]) ** 2
                                            + (wall_y[0] - wall_y[1]) ** 2)
                    sv0 = wall_point_distance(wall_x, wall_y, s[0], wall_length)
                    sv1 = wall_point_distance(wall_x, wall_y, s[1], wall_length)
                    sv3 = wall_point_distance(wall_x, wall_y, s[3], wall_length)

                    for window in sorted_by_name(wall.subSurfaces()):
                        if window.subSurfaceType() not in ('FixedWindow', 'OperableWindow'):
                            continue

                        window_head = ruby_round(max(v.z() for v in window.vertices()), 2)
                        if sv0 == sv1:  # skylight edge 0-1 parallel to the wall
                            if ruby_round(sv0, 2) == ruby_round(d2, 2):
                                length = (skylight_length
                                          + min(0.7 * ceiling_height, d2, d2 - window_head)
                                          + min(0.7 * ceiling_height, d3))
                            elif ruby_round(sv0, 2) == ruby_round(d3, 2):
                                length = (skylight_length + min(0.7 * ceiling_height, d2)
                                          + min(0.7 * ceiling_height, d3, d3 - window_head))
                        elif sv0 == sv3:  # skylight edge 0-3 parallel to the wall
                            if ruby_round(sv0, 2) == ruby_round(d1, 2):
                                width = (skylight_width
                                         + min(0.7 * ceiling_height, d1, d1 - window_head)
                                         + min(0.7 * ceiling_height, d4))
                            elif ruby_round(sv0, 2) == ruby_round(d4, 2):
                                width = (skylight_width + min(0.7 * ceiling_height, d1)
                                         + min(0.7 * ceiling_height, d4, d4 - window_head))
                        _ = head_height
                # #2119: accumulate ONCE per skylight, here — OUTSIDE the
                # exterior-wall/window loops. Before the fix the accumulation sat
                # inside the innermost window loop, so a skylight-only space
                # computed ZERO and each extra exterior window double-counted the
                # whole area. Mirrors necb_2011.rb's `daylighted_under_skylight_area
                # += daylighted_under_skylight_length * daylighted_under_skylight_width`
                # now placed after the `daylight_space.surfaces.sort.each` block.
                area += length * width
            except Exception as e:
                audit.warn('daylighting',
                           f"skylight geometry failed on {sub.nameString()} "
                           f"({type(e).__name__}) — skylight skipped")
    return {'area_m2': area, 'vt_handle': vt_handle, 'skylight_area_m2': skylight_area_sum}


def dist(a, b):
    return math.sqrt((a.x() - b.x()) ** 2 + (a.y() - b.y()) ** 2)


def triangle_height(point, v_a, v_b, base_length):
    area = 0.5 * abs(((point.x() - v_b.x()) * (point.y() - v_a.y()))
                     - ((point.x() - v_a.x()) * (point.y() - v_b.y())))
    return 2.0 * area / base_length


def wall_point_distance(wall_x, wall_y, point, wall_length):
    area = 0.5 * abs(((wall_x[0] - wall_x[1]) * (wall_y[0] - point.y()))
                     - ((wall_x[0] - point.x()) * (wall_y[0] - wall_y[1])))
    return 2.0 * area / wall_length


def costing_area_provider():
    """The costing inversion: btap.costing owns no daylighted-area geometry
    (rule machinery), so the NECB layer hands it this provider."""
    def provider(space):
        return {'sidelighted_m2': sidelighting_parameters(space)['area_m2'],
                'skylight_m2': skylight_parameters(space)['area_m2']}
    return provider
