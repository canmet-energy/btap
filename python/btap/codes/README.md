...
The **umbrella** of the NECB Part 8 performance path. The NECB code family
lives in `necb/`; the umbrella composes its SDK-only domains —

- `necb/hvac/` — reference system selection (Table 8.4.4.7.-A), topology builds,
  capacity-binned efficiencies, ERV, fan rules
- `necb/envelope/` — prescriptive Section 3.2, thermal bridging (3.1.1.7 via TBD),
  the reference-envelope transform (8.4.4.3/.4)
- `necb/loads/` — NECB space-type assignment and space-use loads (occupancy,
  receptacles, ventilation, schedules)
- `necb/lighting/` — Part 4 LPD allowances, daylighting controls, the
