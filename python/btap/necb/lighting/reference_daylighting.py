...
from btap.necb.lighting import daylighting as Daylighting
...
def apply(reference, vintage='2020', proposed=None, placement='necb2020',
    ...
    :param proposed: for (11)(a) set-points
    :param placement: which spaces get photocontrols. 'necb2020' (DEFAULT) =
        where 4.2.2.1.(10)-(15) requires them (D-57); 'necb2011' (alias
        'necb_default') = the legacy NECB 2011 threshold semantics, defects and
        all, kept for the parity gate; 'all' = every daylighted space
    ...
    _set_reference_reflectances(reference, prefix, audit)

    proposed_setpoints = {}
    if proposed is not None:
        for control in proposed.getDaylightingControls():
            if not control.space().is_initialized():
                continue
    ...
    overridden = 0
    for control in sorted_by_name(reference.getDaylightingControls()):
        if not control.space().is_initialized():
            continue

        setpoint = proposed_setpoints.get(control.space().get().nameString())
        if setpoint is None:
            continue
...
def _set_reference_reflectances(reference, prefix, audit):
    """(10)(b): floor 0.15 / wall 0.50 / ceiling 0.80 visible reflectances on
    every interior-facing surface material of the reference."""
    changed = 0
    for surface in sorted_by_name(reference.getSurfaces()):
        reflectance = REFLECTANCES.get(surface.surfaceType())
        if reflectance is None:
            continue
        if (not surface.construction().is_initialized()
                or not surface.construction().get().to_LayeredConstruction().is_initialized()):
            continue

        layers = surface.construction().get().to_LayeredConstruction().get().layers()
        inner = layers[-1].to_OpaqueMaterial()  # inside-facing layer
        if not inner.is_initialized():
            continue

        inner.get().setVisibleAbsorptance(openstudio.OptionalDouble(1.0 - reflectance))
        changed += 1
    audit.decision('lighting_reference',
                   'reference interior visible reflectances set: floor 0.15 / walls 0.50 / ceiling 0.80',
                   inputs={'surfaces': changed}, article=f"{prefix}.5.(10)(b)")
