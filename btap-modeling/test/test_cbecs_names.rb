...
class TestCbecsNames < Minitest::Test
  ...
  def test_baseboard_electric
    ...
    result = BtapModeling.build_system(model, 'Baseboard electric', zones)
  ...
  def test_psz_ac_gas_coil_builds_one_unit_per_zone
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    result = BtapModeling.build_system(model, 'PSZ-AC with gas coil', zones)
  ...
  def test_psz_ac_electric_coil
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'PSZ-AC with electric coil', zones)
  ...
  def test_catalog_filter_by_origin
    cbecs = BtapModeling.systems.select { |r| r['origin'] == 'cbecs' }
