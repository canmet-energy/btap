...
# P1 gate: BtapModeling.characterize round-trips every catalog family (build ->
...
class TestClassify < Minitest::Test
  ...
  def build_and_characterize(name, audit: nil)
    ...
    [model, BtapModeling.characterize(model, audit: audit)]
  end
  ...
  def test_foreign_model_structural_classification
    ...
    facts = BtapModeling.characterize(model)
  ...
  def test_design_cooling_kw_and_audit
    ...
    facts2 = BtapModeling.characterize(model)
