...
class TestVrfErvGshpComposites < Minitest::Test
  ...
  def test_doas_with_vrf_zeroes_terminal_oa
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    result = BtapModeling.build_system(model, 'DOAS with VRF', zones)
  ...
  def test_gshp_variant_ground_hx_no_boiler
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'DOAS with water source heat pumps with ground source heat pump', zones)
  ...
  def test_cooling_tower_wshp_variant
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'DOAS with water source heat pumps cooling tower with boiler', zones)
  ...
  def test_fan_coil_air_cooled_chiller_district_hot_water
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'DOAS with fan coil air-cooled chiller with district hot water', zones)
  ...
  def test_zone_ervs
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'Zone ERVs', zones)
