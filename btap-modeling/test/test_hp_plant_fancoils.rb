...
class TestHpPlantFanCoils < Minitest::Test
  ...
  def test_hs14_gshp_plants_and_fancoils
    ...
    result = BtapModeling.build_system(model, 'hs14_cgshp_fancoils', zones, namer: :necb_pipe_name)
  ...
  def test_hs15_cawhp_companion_heat_pumps
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'hs15_cawhp_fancoils', zones)
  ...
  def test_hs16_uses_ashp_doas
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'hs16_ashp_cawhp_fancoils', zones)
