...
class TestCbecsFamilies < Minitest::Test
  include FixtureHelper

  def test_forced_air_furnace
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    result = BtapModeling.build_system(model, 'Forced air furnace', zones)
  ...
  def test_residential_ac_composite
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    result = BtapModeling.build_system(model, 'Residential AC with baseboard electric', zones)
  ...
  def test_pvav_dx_cooling
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'PVAV with gas boiler reheat', zones)
  ...
  def test_direct_evap_coolers_with_baseboards
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'Direct evap coolers with baseboard electric', zones)
  ...
  def test_doas_with_wshp_composite
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    result = BtapModeling.build_system(model, 'DOAS with water source heat pumps fluid cooler with boiler', zones)
  ...
  def test_doas_with_fan_coils_composite
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    result = BtapModeling.build_system(model, 'DOAS with fan coil chiller with boiler', zones)
