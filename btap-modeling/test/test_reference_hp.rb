...
class TestReferenceHp < Minitest::Test
  ...
  def test_sys3_ashp_dx_heat_with_gas_supplemental
    ...
    result = BtapModeling.build_system(model, SYS3_ASHP, zones, namer: :necb_pipe_name)
  ...
  def test_sys4_ashp_electric_supp_hot_water_baseboards
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, SYS4_ASHP, zones)
  ...
  def test_sys1_ashp_mau_with_cav_reheat_terminals
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    result = BtapModeling.build_system(model, SYS1_ASHP, zones, namer: :necb_pipe_name)
