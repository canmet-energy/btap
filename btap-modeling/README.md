...
Model AUTHORING — the generic, NECB-free half of the btap-*
...
a per-storey 2D floor-plan renderer — plus, since the by-nature refactor
(D-77), the **HVAC authoring machinery**: the 97-system topology catalog,
`build_system`/`replace_system`, twenty system builders, the coil/curve/
...
BtapModeling.create(shape: 'l', length: 40.0, width: 40.0, storeys: 2)
```
...
`BtapModeling.render(model_or_path, path:, height:)` — a port of
...
`BtapModeling.floor_plans(model_or_path, path:, png_dir:, audit:)` — the
...
```ruby
bundle = BtapModeling.floor_plans(model, path: 'floor_plans.html')
...
`BtapModeling::CatalogReport.model_diagrams` returns, so a host report (the
...
OSM that has one) before `BtapNECB::Envelope` passes. With that, the full
authoring chain is gem-only:

geometry → `BtapNECB::Loads.apply_loads` → `BtapNECB::Lighting.apply_lights` →
`BtapNECB::SHW.apply_shw` → `BtapModeling.build_system` →
`BtapNECB::Envelope` prescriptive → `BtapNECB.performance_compliance`
