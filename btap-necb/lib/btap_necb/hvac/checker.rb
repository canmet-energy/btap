  ...
  module HVAC
    ...
    module Checker
      ...
      def check_heat_recovery(model, vintage, hdd, audit)
        ...
        rule = HVAC.rules(vintage)['energy_recovery']
        return if rule.nil?
        ...
        model.getAirLoopHVACs.sort_by(&:nameString).each do |air_loop|
          ...
          supply = HVAC.optional_flow(air_loop.designSupplyAirFlowRate) ||
                   HVAC.optional_flow(air_loop.autosizedDesignSupplyAirFlowRate)
          ctrl = oa_system.get.getControllerOutdoorAir
          min_oa = HVAC.optional_flow(ctrl.minimumOutdoorAirFlowRate) ||
                   HVAC.optional_flow(ctrl.autosizedMinimumOutdoorAirFlowRate)
          ...
          hours = HVAC.annual_availability_hours(air_loop)
          mode = hours.nil? || hours >= rule['continuous_hours_per_year'] ? 'continuous' : 'non_continuous'
          required, threshold_desc = HVAC.erv_threshold_verdict(rule, mode, hdd, oa_pct, supply * 1000.0)
