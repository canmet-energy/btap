  ...
  module Lighting
    ...
    module ApplyLights
      ...
      def apply_to_space_type(model, space_type, vintage, lights_type, lights_scale, audit)
        ...
        unless lpd.zero?
          if lights_type == 'LED'
            led = Lighting.led_record(building_type: building_type, space_type: standards_type)
      ...
      def led_lpd_w_ft2(space_type, standards_type, led, vintage, audit)
        ...
        equations = Lighting.rules(vintage)['atrium_led']
      ...
      def wire_lighting_schedule(model, space_type, record, vintage, audit)
        ...
        lpd = record['lighting_per_area'].to_f
        threshold = Lighting.rules(vintage)['sensor_schedule_lpd_threshold_w_per_ft2'].to_f
      ...
      def emit_article_coverage(vintage, audit)
        BtapAudit::Coverage.emit(Lighting.rules(vintage)['article_coverage'], audit)
      end
