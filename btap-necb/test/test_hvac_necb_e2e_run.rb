...
class TestNecbE2ERun < Minitest::Test
  ...
  def test_proposed_and_sys3_reference_size_cleanly
    ...
    result = BtapNECB::HVAC.reference_hvac(proposed,
    ...
    BtapNECB::HVAC.apply_efficiencies(result.model, vintage: '2020', audit: audit)
  ...
  def test_ashp_reference_conditions_through_a_cold_week
    proposed = attach_weather!(load_fixture)
    zones = proposed.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(proposed, 'PTHP', zones)
    ...
    BtapNECB::HVAC.apply_efficiencies(result.model, vintage: '2020')
  ...
  def test_sys6_reference_with_erv_simulates_a_week
    proposed = attach_weather!(load_fixture)
    zones = proposed.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(proposed, 'MZ BU RTU Hot Water Heating Coil Scroll Chiller and Hot Water Baseboard', zones)
    ...
    erv_audit = BtapNECB::HVAC.apply_energy_recovery(result.model, hdd: 3890)
