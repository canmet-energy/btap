...
require_relative '../lib/btap_necb/cli'
require 'stringio'
require 'tmpdir'
...
class TestCLI < Minitest::Test
  ...
  def run_cli(*argv)
    BtapNECB::CLI.run(argv, out: @out, err: @err)
  end
  ...
  def test_preflight_error_is_an_argument_error_so_existing_rescues_still_work
    assert_operator(BtapNECB::PreflightError, :<, ArgumentError)
  end
  ...
  def test_city_resolves_to_a_file_whose_ddy_and_stat_sit_beside_it
    epw = BtapNECB::CLI::Weather.available['toronto']
  ...
  def test_weather_is_discoverable_in_the_PACKAGED_layout
    Dir.mktmpdir do |root|
      gem_lib = File.join(root, 'gems/btap-necb/lib/btap_necb')
      ...
      found = BtapNECB::CLI::Weather.search
      ...
      assert_includes(found, File.expand_path('../../../../weather', __dir__.sub('/test', '/lib/btap_necb')),
