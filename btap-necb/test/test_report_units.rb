...
class TestReportUnits < Minitest::Test
  H = BtapNECB::Report::Html
  Checklist = BtapNECB::Report::Checklist
  Charts = BtapNECB::Report::Charts
  Sections = BtapNECB::Report::Sections
  ...
  def canned_result
    BtapNECB::Compliance::ComplianceResult.new(
  ...
  def test_coverage_reconciliation
    ...
    html = BtapNECB::Report.render(canned_result)
  ...
  def test_rulings_appendix_renders_fired_decisions
    ...
    assert_includes html, H.esc(BtapNECB::Decisions.lookup('D-43')['title'])
    assert_includes html, H.esc(BtapNECB::Decisions.lookup('D-19')['summary'][0, 40])
    ...
    assert_includes BtapNECB::Report.render(canned_result), %(<tr id="audit-#{first}">)
  ...
  def test_paired_bars_golden
    ...
    assert_includes svg, BtapNECB::Report::Html::PROPOSED_COLOR
    assert_includes svg, BtapNECB::Report::Html::REFERENCE_COLOR
  ...
  def test_paired_bars_edge_cases
    ...
    refute_includes solo, BtapNECB::Report::Html::REFERENCE_COLOR, 'nil reference renders proposed-only'
  end
  ...
  def test_full_render_from_canned_data
    html = BtapNECB::Report.render(canned_result, project_name: 'Unit Test Tower',
  ...
  def test_shortened_run_warns_loudly
    ...
    result = BtapNECB::Compliance::ComplianceResult.new(
      proposed_model: nil, reference_model: nil, report: report,
      audit: canned_audit, compliant: true, run_dir: nil)
    html = BtapNECB::Report.render(result)
  ...
  def test_undetermined_run
    ...
    result = BtapNECB::Compliance::ComplianceResult.new(
      proposed_model: nil, reference_model: nil, report: report,
      audit: BtapNECB::AuditLog.new, compliant: nil, run_dir: nil)
    html = BtapNECB::Report.render(result)
