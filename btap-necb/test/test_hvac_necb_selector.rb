...
class TestNecbSelector < Minitest::Test
  ...
  def test_rules_provenance_and_catalog_names_resolve
    rules = BtapNECB::HVAC.rules('2020')
    ...
    rules['system_definitions'].each do |key, defn|
      ...
      %w[gas electric].each do |fuel|
        ...
        assert BtapModeling::Catalog.resolve(name), "unresolvable catalog name for system #{key}/#{fuel}"
      end
    end
  end
  ...
  def test_museum_exhibition_is_assembly_and_restoration_is_collections
    ...
    ruleset = Marshal.load(Marshal.dump(BtapNECB::HVAC.rules('2020')))
