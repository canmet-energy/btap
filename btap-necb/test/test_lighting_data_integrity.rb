...
class TestDataIntegrity < Minitest::Test
  def test_led_table_matches_legacy_shape
    led = BtapNECB::Lighting.table('led_lighting_2020')
    assert_equal 308, led.size
    record = BtapNECB::Lighting.led_record(building_type: 'Space Function',
  ...
  def test_2025_space_function_table
    rows = BtapNECB::Lighting.table('lpd_space_functions_2025')
  ...
  def test_2025_building_type_table
    rows = BtapNECB::Lighting.table('lpd_building_types_2025')
  ...
  def test_rules_and_coverage_lint
    %w[2020 2025].each do |vintage|
      rules = BtapNECB::Lighting.rules(vintage)
    ...
    assert_equal '2020', BtapNECB::Lighting.data_vintage('2025')
    assert_match(/zero LPD differences|ZERO value differences/i,
                 BtapNECB::Lighting.rules('2025')['provenance']['method'])
  end
end
