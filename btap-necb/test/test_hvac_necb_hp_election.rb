...
class TestNecbHpElection < Minitest::Test
  ...
  def test_gate_passes_and_the_largest_terminal_fuel_is_elected
    ...
    elected = BtapNECB::HVAC.heat_pump_aux_energy_type(g, facts_for(g), HP_RULES, annual, audit)
  ...
  def test_no_terminal_heating_falls_back
    ...
    assert_nil BtapNECB::HVAC.heat_pump_aux_energy_type(g, facts_for(g), HP_RULES, annual, audit)
  ...
  def test_no_annual_data_falls_back_with_the_sizing_note
    audit = BtapNECB::AuditLog.new
    g = group
    assert_nil BtapNECB::HVAC.heat_pump_aux_energy_type(g, facts_for(g), HP_RULES, nil, audit)
  ...
  def test_g_ii_aggregates_over_groups_sharing_the_source_water_loop
    ...
    annual = { loops: { 'Loop 1' => { hp_j: 40e9, aux: [{ fuel: 'Electricity', j: 10e9 }] },
               ...
               zones: {} }
    elected = BtapNECB::HVAC.heat_pump_aux_energy_type(a, facts_for(a, b), HP_RULES, annual, audit)
  ...
  def test_election_wires_into_finalize_through_reference_hvac
    ...
    BtapModeling.build_system(model, SYS3_ASHP, zones)
    ...
    without = BtapNECB::HVAC.reference_hvac(load_and_build, vintage: '2020',
                                                  audit: BtapNECB::AuditLog.new)
  ...
  def load_and_build
    model = load_fixture
    BtapModeling.build_system(model, SYS3_ASHP, model.getThermalZones.sort_by(&:nameString))
    model
  end
  ...
  def test_inventory_finds_hp_aux_and_baseboards_on_a_real_build
    ...
    inventory = BtapModeling::Classify.heating_election_inventory(model)
    ...
    baseboards = inventory[:zones].values.flatten.select { |e| e[:variable] == BtapModeling::Classify::BASEBOARD_VARIABLE }
