...
class TestNecb2025 < Minitest::Test
  ...
  def select(groups, zone_types:, storeys: 1, vintage: '2025')
    BtapNECB::HVAC.select_reference_systems(
  ...
  def test_2025_article_renumbering
    ...
    rules = BtapNECB::HVAC.rules('2025')
  ...
  def test_reference_hvac_2025_native_efficiencies
    ...
    result = BtapNECB::HVAC.reference_hvac(model, vintage: '2025',
  ...
  def test_effective_vintage_native_for_both
    %w[2020 2025].each do |v|
      vintage, reason = BtapNECB::HVAC::Efficiency.effective_vintage(v)
  ...
  def test_2025_small_heat_pump_cooling_is_seer_15
    ...
    %w[2020 2025].each do |vintage|
      model = load_fixture
      zones = model.getThermalZones.sort_by(&:nameString)
      BtapModeling.build_system(model, 'PSZ RTU ASHP with Electric and ASHP with Electric Supp. Heat Coils and Electric Baseboard', zones)
      ...
      BtapNECB::HVAC.apply_efficiencies(model, vintage: vintage)
  ...
  def test_2025_boiler_and_chiller_values_unchanged
    ...
    BtapNECB::HVAC.apply_efficiencies(model, vintage: '2025')
