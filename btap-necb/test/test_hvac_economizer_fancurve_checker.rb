...
class TestEconomizerFancurveChecker < Minitest::Test
  ...
  def test_fan_power_curve_applied_after_sizing
    ...
    audit = BtapNECB::AuditLog.new
    BtapNECB::HVAC.apply_efficiencies(reference, vintage: '2020', audit: audit)
  ...
  def test_part5_checker
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'PSZ RTU with exhaust Gas and DX Coils and Hot Water Baseboard', zones)
    ...
    audit = BtapNECB::HVAC.check_part5(model, vintage: '2020', hdd: 3890)
