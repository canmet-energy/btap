...
class TestTiersEUI < Minitest::Test
  ...
  def test_tier_arithmetic
    ...
    assert_equal 1, BtapNECB::Tiers.energy_tier(95_000, 100_000, audit: audit)['tier']
    assert_equal 2, BtapNECB::Tiers.energy_tier(70_000, 100_000)['tier']
    assert_equal 3, BtapNECB::Tiers.energy_tier(50_000, 100_000)['tier']
    assert_equal 4, BtapNECB::Tiers.energy_tier(39_000, 100_000)['tier']
    assert_nil BtapNECB::Tiers.energy_tier(101_000, 100_000)['tier']
    assert(audit.entries.any? { |e| e[:article].to_s.include?('10.1.2.1') })
  end

  def test_eui_bet_arithmetic
    audit = BtapNECB::AuditLog.new
    target = BtapNECB::Tiers.eui_building_energy_target(
    ...
    assert_raises(ArgumentError) do
      BtapNECB::Tiers.eui_building_energy_target({ 'Casino' => 100 }, 1600.0, hdd: 3890)
    end
    cold = BtapNECB::AuditLog.new
    BtapNECB::Tiers.eui_building_energy_target({ 'Office' => nil }, 1600.0, hdd: 9500, audit: cold)
  ...
  def test_ghg_levels
    ...
    kg = BtapNECB::Tiers.operational_ghg_kg(energy, 'ONTARIO')
    assert_in_delta (10_000 * 57.9 + 50_000 * 185) / 1000.0, kg, 0.1
    audit = BtapNECB::AuditLog.new
    level = BtapNECB::Tiers.ghg_level(kg, kg * 4, audit: audit)
    assert_equal 'B', level['level'], '25% of target -> level B (<= 25%)'
    assert_equal 'F', BtapNECB::Tiers.ghg_level(99.0, 100.0)['level']
    assert_nil BtapNECB::Tiers.ghg_level(101.0, 100.0)['level']
  end
  ...
  def test_eui_path_guards
    assert_raises(ArgumentError) do
      BtapNECB.performance_compliance(proposed_with_hvac, vintage: '2020', path: :eui,
