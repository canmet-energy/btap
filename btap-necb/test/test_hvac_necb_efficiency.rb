...
class TestNecbEfficiency < Minitest::Test
  ...
  def test_dx_and_gas_coil_and_ashp
    model = load_fixture
    zones = model.getThermalZones.sort_by(&:nameString)
    BtapModeling.build_system(model, 'PSZ RTU Gas and DX Coils and Electric Baseboard', zones)
    ...
    BtapNECB::HVAC.apply_efficiencies(model, vintage: '2020')
    ...
    BtapModeling.build_system(model2, 'PSZ RTU ASHP with Electric and ASHP with Electric Supp. Heat Coils and Electric Baseboard', zones2)
    ...
    BtapNECB::HVAC.apply_efficiencies(model2, vintage: '2020')
