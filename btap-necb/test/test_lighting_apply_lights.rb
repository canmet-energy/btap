...
class TestApplyLights < Minitest::Test
  ...
  def test_plain_lpd_below_threshold
    ...
    audit = BtapNECB::Lighting.apply_lights(model, vintage: '2020', audit: BtapNECB::AuditLog.new)
  ...
  def test_led_path_and_atrium_height_fix
    ...
    BtapNECB::Lighting.apply_lights(model, vintage: '2020', lights_type: 'LED',
                                    audit: BtapNECB::AuditLog.new)
    led = BtapNECB::Lighting.led_record(building_type: OFFICE[0], space_type: OFFICE[1])
    lights = st.lights.first
    ...
    audit = BtapNECB::AuditLog.new
    BtapNECB::Lighting.apply_lights(atrium_model, vintage: '2020', lights_type: 'LED', audit: audit)
  ...
  def test_scale_and_coverage
    ...
    st = tagged_space_type(model, *OFFICE)
    audit = BtapNECB::AuditLog.new
    BtapNECB::Lighting.apply_lights(model, vintage: '2020', lights_scale: 0.5, audit: audit)
    record = BtapNECB::Loads::SpaceTypes.record(building_type: OFFICE[0], space_type: OFFICE[1])
