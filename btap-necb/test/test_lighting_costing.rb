...
class TestCosting < Minitest::Test
  ...
  def costed_fixture_model(lights_type: 'NECB_Default')
    ...
    BtapNECB::Lighting.apply_lights(model, vintage: '2020', lights_type: lights_type)
    model
  end

  def test_fixture_costing_end_to_end
    audit = BtapNECB::AuditLog.new
    report = BtapNECB::Lighting.cost(costed_fixture_model, vintage: '2020',
  ...
  def test_cfl_request_falls_back_to_led_only_2020_catalog
    audit = BtapNECB::AuditLog.new
    cfl = BtapNECB::Lighting.cost(costed_fixture_model(lights_type: 'NECB_Default'),
                                  vintage: '2020', city: CITY, province_state: PROVINCE, audit: audit)
    led = BtapNECB::Lighting.cost(costed_fixture_model(lights_type: 'LED'),
  ...
  def test_daylighting_sensors_costed
    ...
    audit = BtapNECB::AuditLog.new
    created = BtapNECB::Lighting.add_daylighting_controls(model, vintage: '2020', audit: audit)
    ...
    base = BtapNECB::Lighting.cost(costed_fixture_model, vintage: '2020', city: CITY, province_state: PROVINCE)
    report = BtapNECB::Lighting.cost(model, vintage: '2020', city: CITY, province_state: PROVINCE, audit: audit)
