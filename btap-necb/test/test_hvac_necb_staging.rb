...
class TestNecbStaging < Minitest::Test
  ...
  def build(name, config: STAGED, weather: false)
    ...
    BtapModeling.build_system(model, name, zones, control_zone: zones.first, config: config)
    [model, zones]
  end
  ...
  def rules
    BtapNECB::HVAC.rules('2020')
  end

  def stage_caps(coil)
    coil.stages.map do |s|
      if s.respond_to?(:grossRatedTotalCoolingCapacity)
        BtapNECB::HVAC::Efficiency.optional_f(s.grossRatedTotalCoolingCapacity) ||
          BtapNECB::HVAC::Efficiency.optional_f(s.autosizedGrossRatedTotalCoolingCapacity)
      elsif s.respond_to?(:grossRatedHeatingCapacity)
        BtapNECB::HVAC::Efficiency.optional_f(s.grossRatedHeatingCapacity) ||
          BtapNECB::HVAC::Efficiency.optional_f(s.autosizedGrossRatedHeatingCapacity)
      else
        BtapNECB::HVAC::Efficiency.optional_f(s.nominalCapacity) ||
          BtapNECB::HVAC::Efficiency.optional_f(s.autosizedNominalCapacity)
      end
    end
  end
  ...
  def test_staged_gas_psz_builds_a_unitary_with_two_autosized_stages
    ...
    cooling = BtapModeling::Coils.multispeed(unitary.coolingCoil)
    heating = BtapModeling::Coils.multispeed(unitary.heatingCoil)
  ...
  def test_supply_components_descends_into_the_unitary
    ...
    deep = BtapModeling::Coils.supply_components(air_loop).map { |c| c.iddObjectType.valueName }
  ...
  def test_stage_count_follows_the_vendored_thresholds
    ...
    counts = [10.0, 66.0, 66.1, 132.0, 132.1, 198.0, 264.0, 500.0].map do |kw|
      ...
      [wanted, BtapNECB::HVAC::Efficiency::MAX_STAGES].min
    end
  ...
  def test_economizer_staging_floor_5_2_2_8
    ...
    { 100.0 => 4, 40.0 => 2, 20.0 => 2 }.each do |kw, expected|
      model, = build(GAS_PSZ)
      unitary = model.getAirLoopHVACUnitarySystems.first
      coil = BtapModeling::Coils.multispeed(unitary.coolingCoil)
      ...
      BtapNECB::HVAC::Efficiency.send(:apply_staging, model, rules, '2020', audit)
    ...
    BtapNECB::HVAC::Efficiency.send(:apply_staging, model, rules, '2020', BtapNECB::AuditLog.new)
  ...
  def test_stage_count_can_be_raised_after_sizing_and_recapacitates
    ...
    unitary = model.getAirLoopHVACUnitarySystems.first
    BtapNECB::HVAC::Efficiency.resize_stages(cooling, 3)
    BtapModeling::Coils.set_stage_flow_ratios(unitary)
  ...
  def test_clamp_at_the_four_stage_ceiling_warns_loudly
    ...
    audit = BtapNECB::AuditLog.new
    BtapNECB::HVAC::Efficiency.apply_staging(model, rules, '2020', audit)
  ...
  def test_reference_transform_audits_the_split_for_both_shapes
    ...
    result = BtapNECB::HVAC.reference_hvac(model, building: { storeys: 1, zone_types: types })
  ...
  def test_stage_flow_ratios_are_floored_at_the_outdoor_air_fraction
    ...
    coil = BtapModeling::Coils.dx_cooling_multi_speed(model, model.alwaysOnDiscreteSchedule)
    ...
    BtapModeling::Coils.set_stage_flow_ratios(unitary, min_ratio: 0.7)
    ...
    BtapModeling::Coils.set_stage_flow_ratios(unitary, min_ratio: 0.2)
    assert_in_delta 0.5, performance.supplyAirflowRatioFields.first.coolingRatio.get, 1e-6
    BtapModeling::Coils.set_stage_flow_ratios(unitary, min_ratio: 1.5)
