...
class TestDaylightingNecb2020 < Minitest::Test
  DA = BtapNECB::Lighting::DaylightedAreas
  REQ = BtapNECB::Lighting::DaylightControlRequirement
  ...
  def box(windows: [], skylights: [], width: 10.0, depth: 8.0, height: 3.0,
    ...
    if space_type
      ...
      BtapNECB::Loads.assign_space_types(model, { space.nameString => ['Space Function', space_type] },
                                         vintage: '2020')
      BtapNECB::Lighting.apply_lights(model, vintage: '2020')
    end
    [model, space]
  end
  ...
  def test_skylight_only_space_gets_toplighted_area
    ...
    legacy = BtapNECB::Lighting::Daylighting.skylight_parameters(space)
  ...
  def test_controls_are_placed_with_a_daylighted_area_fraction_not_1_0
    model, space = box(windows: [[0.0, 10.0, 0.0, 3.0]], space_type: 'Office enclosed > 25 m2')
    audit = BtapNECB::AuditLog.new
    created = BtapNECB::Lighting.add_daylighting_controls(model, vintage: '2020', placement: :necb2020,
                                                          audit: audit)
  ...
  def test_legacy_2011_placement_is_still_reachable_and_shouts
    ...
    audit = BtapNECB::AuditLog.new
    created = BtapNECB::Lighting.add_daylighting_controls(model, vintage: '2020',
  ...
  def test_reference_daylighting_defaults_to_the_2020_rule
    model, = box(windows: [[0.0, 10.0, 0.0, 3.0]], space_type: 'Office enclosed > 25 m2')
    audit = BtapNECB::AuditLog.new
    BtapNECB::Lighting.reference_daylighting(model, vintage: '2020', audit: audit)
    ...
    BtapNECB::Lighting.reference_daylighting(legacy_model, vintage: '2020', placement: :necb_default,
                                                  office_match: :legacy)
  ...
  def test_unevaluated_1500_hour_exception_is_declared_every_run
    model, = box(windows: [[0.0, 10.0, 0.0, 3.0]], space_type: 'Office enclosed > 25 m2')
    audit = BtapNECB::AuditLog.new
    BtapNECB::Lighting.add_daylighting_controls(model, vintage: '2020', placement: :necb2020, audit: audit)
  ...
  def test_deprecated_option_alias_still_selects_the_same_rule
    ...
    model, = box(windows: [[0.0, 10.0, 0.0, 3.0]], space_type: 'Office enclosed > 25 m2')
    audit = BtapNECB::AuditLog.new
    created = BtapNECB::Lighting.add_daylighting_controls(model, vintage: '2020', option: 'NECB_Default',
                                                          audit: audit)
    assert_equal 1, created
    ...
    assert_equal 1, BtapNECB::Lighting.add_daylighting_controls(legacy, vintage: '2020', option: 'NECB_Default',
                                                                placement: :necb2011)
    ...
    blanket_audit = BtapNECB::AuditLog.new
    assert_equal 1, BtapNECB::Lighting.add_daylighting_controls(blanket, vintage: '2020', option: 'all',
  ...
  def test_placement_default_is_the_blanket_rule_and_unknowns_raise
    ...
    assert_raises(ArgumentError) { BtapNECB::Lighting.add_daylighting_controls(other, placement: :necb2050) }
  end
end
