...
class TestNecbReference < Minitest::Test
  ...
  def test_small_office_to_sys3_electric
    ...
    result = BtapNECB::HVAC.reference_hvac(model, building: { storeys: 1, zone_types: types(model, 'Office - enclosed') })
  ...
  def test_three_storey_office_to_sys6
    ...
    result = BtapNECB::HVAC.reference_hvac(model, building: { storeys: 3, zone_types: types(model, 'Office - open plan') })
  ...
  def test_residential_pthp_redirects_to_ashp_reference
    model = proposed('PTHP')
    result = BtapNECB::HVAC.reference_hvac(model, building: { storeys: 1, zone_types: types(model, 'Multi-unit residential') })
  ...
  def test_water_loop_hp_keeps_selection_ground_source_redirects
    ...
    result = BtapNECB::HVAC.reference_hvac(model, audit: audit,
    ...
    result = BtapNECB::HVAC.reference_hvac(ground,
  ...
  def test_data_centre_to_sys2
    ...
    result = BtapNECB::HVAC.reference_hvac(model, building: { storeys: 1, zone_types: types(model, 'Data centre') })
