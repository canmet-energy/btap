...
class TestHPInstantaneous < Minitest::Test
  ...
  def test_instantaneous_bins
    ...
    BtapNECB::SHW.apply_water_heater_efficiency(small_gas, vintage: '2020', audit: audit)
    ...
    BtapNECB::SHW.apply_water_heater_efficiency(big_gas, vintage: '2020', audit: audit)
    ...
    BtapNECB::SHW.apply_water_heater_efficiency(oil, vintage: '2020', audit: audit)
  ...
  def test_heat_pump_water_heater_build_and_floor
    model = tagged_model
    audit = BtapNECB::AuditLog.new
    loop = BtapNECB::SHW.apply_shw(model, vintage: '2020', fuel: 'HeatPump', audit: audit)
  ...
  def test_2025_uef_floor
    model = tagged_model
    BtapNECB::SHW.apply_shw(model, vintage: '2025', fuel: 'HeatPump')
  ...
  def test_hphw_costing_detection
    model = tagged_model
    BtapNECB::SHW.apply_shw(model, vintage: '2020', fuel: 'HeatPump')
    audit = BtapNECB::AuditLog.new
    report = BtapNECB::SHW.cost(model, city: 'TORONTO', province_state: 'ONTARIO', audit: audit)
