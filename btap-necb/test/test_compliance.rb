...
class TestCompliance < Minitest::Test
  ...
  def test_annual_mode_week_run_full_determination
    ...
    assert_includes rulings.flat_map { |r| BtapNECB::Decisions.ids_in(r) }.uniq, 'D-14',
                    'D-14 (operating-schedule inheritance) fired and was tagged'
    rulings.flat_map { |r| BtapNECB::Decisions.ids_in(r) }.uniq.each do |id|
      refute_nil BtapNECB::Decisions.lookup(id), "audited ruling #{id} resolves in the registry"
    end
  ...
  def test_input_model_validation_gates
    ...
    e = assert_raises(ArgumentError) do
      BtapNECB.performance_compliance('/nope/missing.osm', vintage: '2020',
    ...
    e = assert_raises(ArgumentError) do
      BtapNECB.performance_compliance(OpenStudio::Model::Model.new, vintage: '2020',
                                            simulate: :none, hdd: 3890, run_dir: dir)
    end
    ...
    e = assert_raises(ArgumentError) do
      BtapNECB.performance_compliance(bare, vintage: '2020', simulate: :none, hdd: 3890, run_dir: dir)
    end
    ...
    e = assert_raises(ArgumentError) do
      BtapNECB.performance_compliance(no_storeys, vintage: '2020', simulate: :none, hdd: 3890, run_dir: dir)
    end
    assert_includes e.message, 'ABOVE-GROUND STOREY COUNT'
    result = BtapNECB.performance_compliance(no_storeys, vintage: '2020', simulate: :none, hdd: 3890,
  ...
  def test_next_sizing_factor_secant_and_fallback
    call = ->(history, f, h, target, step) do
      BtapNECB::Compliance.send(:next_sizing_factor, history, f, h, target, step)
    end
  ...
  def test_bump_capacities_targets_failing_zones
    ...
    trace = {}
    factors = BtapNECB::Compliance.send(
