...
# start the measure
class BTAPSetFanEff < OpenStudio::Measure::ModelMeasure
  ...
  def name
    ...
    return "BTAPSetFanEff"
  end

  # human readable description
  def description
    return "Sets all fan efficiency to specified value. Does not curently work with Fan:SystemModel"
  end
  ...
  def modeler_description
    ...
    def arguments(model)
      ...
      eff_for_this_cz.setDefaultValue(0.72)
  ...
  def run(model, runner, user_arguments)
    ...
    if eff_for_this_cz == 999
      runner.registerInfo("BTAPSetFanEff is skipped")

    else
      runner.registerInfo("BTAPSetFanEff is not skipped")
...
BTAPSetFanEff.new.registerWithApplication
