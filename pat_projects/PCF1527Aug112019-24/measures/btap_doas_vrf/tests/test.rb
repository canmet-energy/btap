...
class BTAPDOASVRFTest < Minitest::Test
  # def setup
  # end
  ...
  def test_number_of_arguments_and_argument_names
    ...
    assert_equal('loops_to_change', arguments[0].name)
  end
      ...
      terminal =zone.airLoopHVACTerminal.get.to_HVACComponent.get
        zone_lists.each do |zone_list|
          if zone_list.thermalZone == zone
            ...
            assert(zone_list.equipmentInHeatingOrder[1].name.to_s.include?("Variable Refrigerant Flow"),"VRF terminal is not 2nd in cooling order")
          end
        end
      end
    end
