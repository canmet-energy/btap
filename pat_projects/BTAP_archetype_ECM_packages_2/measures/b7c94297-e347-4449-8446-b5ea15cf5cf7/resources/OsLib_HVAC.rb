 ...
 def OsLib_HVAC.removeEquipment(model, runner, options)
  ...
  def OsLib_HVAC.createHotWaterPlant(model, runner, hot_water_setpoint_schedule, loop_type, parameters)
    ...
    loop_sizing.setLoopType("Heating")
     loop_sizing.setDesignLoopExitTemperature(OpenStudio::convert(parameters["hw_loop_temp"],"F","C").get) #ML follows convention of sizing temp being larger than supplu temp 
    ...
    # create a boiler
		...
		boiler.setDesignWaterOutletTemperature(OpenStudio::convert(parameters["boilerHWST"],"F","C").get)
    # create a scheduled setpoint manager
		hw_t_c = OpenStudio::convert(parameters["hw_loop_temp"],"F","C").get
    ...
    hw_t_sch.defaultDaySchedule().addValue(OpenStudio::Time.new(0,24,0,0),hw_t_c)  
		setpoint_manager_scheduled = OpenStudio::Model::SetpointManagerScheduled.new(model,hw_t_sch)
  ...
  def OsLib_HVAC.createChilledWaterPlant(model, runner, chilled_water_setpoint_schedule, loop_type, chillerType, parameters)
    ...
    loop_sizing.setLoopType("Cooling")
      loop_sizing.setDesignLoopExitTemperature(OpenStudio::convert(parameters["chw_loop_temp"],"F","C").get)   
    ...
    if chillerType == "WaterCooled"
      ...
      chiller.setReferenceCOP(parameters["chiller_cop"])
			chiller.setReferenceLeavingChilledWaterTemperature(OpenStudio::convert(parameters["chillerCHWST"],"F","C").get)
    ...
    # create a scheduled setpoint manager
		chw_t_c = OpenStudio::convert(parameters["chw_loop_temp"],"F","C").get
    ...
    setpoint_manager_scheduled = OpenStudio::Model::SetpointManagerScheduled.new(model,chw_t_sch)
    # create a supply bypass pipe
    pipe_supply_bypass = OpenStudio::Model::PipeAdiabatic.new(model)
    # create a supply outlet pipe
    pipe_supply_outlet = OpenStudio::Model::PipeAdiabatic.new(model)
  ...
  def OsLib_HVAC.createPrimaryAirLoops(model, runner, options, parameters)
    ...
    model.getBuildingStorys.sort.each do |building_story|
      ...
      unless thermalZonesToAdd.empty?
        ...
        airloop_primary.setName("DOAS Chilled Beam - #{building_story.name}")
        ...
        if options["primaryHVAC"]["cool"] == "Water"
          ...
          cooling_coil = OpenStudio::Model::CoilCoolingDXSingleSpeed.new(model,
                                                                           ...
                                                                           clgPlrCurve)
          cooling_coil.setRatedCOP(OpenStudio::OptionalDouble.new(parameters["dx_cop"]))
          air_loop_comps << cooling_coil
        elsif options["primaryHVAC"]["cool"] == "TwoSpeedDX"
          ...
          cooling_coil = OpenStudio::Model::CoilCoolingDXTwoSpeed.new(model,
                                                                      ...
                                                                      clgCapFuncTempCurve,
                                                                      clgEirFuncTempCurve)
          cooling_coil.setRatedHighSpeedCOP(OpenStudio::OptionalDouble.new(parameters["dx_cop"]))
          cooling_coil.setRatedLowSpeedCOP(OpenStudio::OptionalDouble.new(parameters["dx_cop"]))
          air_loop_comps << cooling_coil
				else
					#no cooling coil
        end
        ...
        thermalZonesToAdd.each do |zone|
          # make an air terminal for the zone
					coil_cooled_beam = OpenStudio::Model::CoilCoolingCooledBeam.new(model)
					air_terminal = OpenStudio::Model::AirTerminalSingleDuctConstantVolumeCooledBeam.new(model, model.alwaysOnDiscreteSchedule(), coil_cooled_beam)
					air_terminal.setCooledBeamType(parameters["chilled_beam_type"])
					options["chilled_water_plant"].addDemandBranchForComponent(coil_cooled_beam)
      ...
      if options["zonesPrimary"].include? zone
        if options["zoneHVAC"] == "Baseboard"
          ...
          cooling_coil = OpenStudio::Model::CoilCoolingLowTempRadiantVarFlow.new(model, options["mean_radiant_cooling_setpoint_schedule"])
					cooling_coil.setMaximumColdWaterFlow(0)
