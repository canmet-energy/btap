...
class BTAPResults < OpenStudio::Ruleset::ReportingUserScript
  ...
  def run(runner, user_arguments)
    ...
    # Temporary output until qaqc is fixed for NECB2015 and NECB2017
    ...
    puts "\nFile cost_results.json successfully created in output folder.\n\n"
    ...
    store_data(runner,  qaqc[:costing_information] ,"costing_information", "?")

    #adding this values for optimization runs.
    runner.registerValue('total_envelope_cost', cost_result["totals"]["envelope"].to_f)
    runner.registerValue('total_lighting_cost', cost_result["totals"]["lighting"].to_f)
    runner.registerValue('total_heating_cooling_cost', cost_result["totals"]["heating & cooling"].to_f)
    runner.registerValue('total_shw_cost', cost_result["totals"]["shw"].to_f)
    runner.registerValue('total_ventilation_cost', cost_result["totals"]["ventilation"].to_f)
    runner.registerValue('total_capital_cost', cost_result["totals"]["grand total"].to_f)
    runner.registerValue('total_neb_utility_costs', qaqc[:economics][:total_neb_cost].to_f)
    runner.registerValue('total_capital_and_energy_costs', cost_result["totals"]["grand total"].to_f + qaqc[:economics][:total_neb_cost].to_f)
    runner.registerValue('total_envelope_and_energy_costs', cost_result["totals"]["envelope"].to_f + qaqc[:economics][:total_neb_cost].to_f )
    # reporting final condition
