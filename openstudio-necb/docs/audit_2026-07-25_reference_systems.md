...
| A2 | Water-LOOP vs water-SOURCE HP boundary (8.4.4.13.(1) keeps WLHP on Table -A; sentence (2) redirects W-SOURCE). Our single hp flag redirects both; Note A-8.4.4.13 unavailable via MCP (Appendix A never ingested — filed as canmet-energy/hbix#67, 2026-07-27). Touches the WSHP e2e test premise. **Legacy checked (2026-07-27): legacy is SILENT — necb_reference_hp is a global generation-input boolean, no per-block HP detection, no WLHP/WSHP distinction anywhere; 'follow legacy' is not an available option here.** **Note A-8.4.4.13 NOW RETRIEVED (2026-07-28, D-35 — appendix ingested per hbix#67): water-LOOP = HP on an INTERNAL water loop (aux boiler and/or cooling tower allowed); water-SOURCE = a water-loop system whose source is EXTERNAL water (surface water, groundwater, or a loop carrying waste heat generated outside the building, directly or via HX); ground-source = ground HX. RECOMMENDED RULING: implement the printed split — HP whose source loop is internal boiler+tower stays on Table -A (8.4.4.13.(1)); ground HX / district-source / external-water redirects (8.4.4.13.(2)); detectable from the model by inspecting the HP source plant loop. Awaiting phylroy.** |
