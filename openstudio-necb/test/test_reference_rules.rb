...
class TestReferenceRules < Minitest::Test
  ...
  def sample(slug)
    ...
    skip("sample not generated: run ruby openstudio-necb/scripts/generate_samples.rb") unless File.exist?(path)
    path
  end

  def compliance(slug, dir, **kwargs)
    OpenStudioNECB.performance_compliance(sample(slug), vintage: '2020', hdd: 3890,
  ...
  def test_auxiliary_fuel_election_runs_on_an_annual_run
    ...
    Dir.mktmpdir do |dir|
      r = OpenStudioNECB.performance_compliance(
