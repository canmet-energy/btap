...
module BtapCosting
  module Lighting
    ...
    module Fixtures
      ...
      def cost(model, database:, vintage:, province_state:, city:, audit:, daylighting_areas: nil)
      ...
      def daylighting_note(model, database, template, province_state, city, section, audit, daylighting_areas: nil)
      ...
      def zone_daylighting_data(zone, database, template, daylighting_areas)
        ...
        zone.spaces.sort_by(&:nameString).each do |space|
          ...
          areas = daylighting_areas.call(space)
          sidelighted += areas[:sidelighted_m2].to_f
          skylight += areas[:skylight_m2].to_f
        end
