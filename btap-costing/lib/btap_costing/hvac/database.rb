...
module BtapCosting
  module HVAC
    ...
    # data/hvac/README.md) with runtime injection of licensed values via costs_csv.
    ...
    class Database
      ...
      DATA_DIR = File.expand_path('../data/hvac', __dir__)
      ...
      def initialize(costs_csv: nil)
        @warnings = []
        @costs = index_by_id(load_csv(File.join(SHARED_DIR, 'costs.csv')))
        if costs_csv
        ...
        @local_factors = load_csv(File.join(SHARED_DIR, 'costs_local_factors.csv'))
