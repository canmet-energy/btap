...
class BTAPCreateGeometry < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
    ...
    # Generate the geometry
       model=BTAP::Geometry::Wizards::create_shape_courtyard(model, length = a, width = b, courtyard_length = a/3, courtyard_width = b/3, num_floors = arguments['above_grade_floors'], floor_to_floor_height = arguments['floor_to_floor_height'], plenum_height = arguments['plenum_height'], perimeter_zone_depth = perimeter_depth)
       BTAP::FileIO::save_osm(model, File.join(File.dirname(__FILE__), "output", "courtyard.osm"))
       osm_model_path = File.absolute_path("../output/courtyard.osm")
      ...
      osm_model_path = File.absolute_path("../output/rectangular.osm")
      ...
      BTAP::FileIO::save_osm(model, File.join(File.dirname(__FILE__), "output", "Lshape.osm"))
      osm_model_path = File.absolute_path("../output/Lshape.osm")
      ...
      BTAP::FileIO::save_osm(model, File.join(File.dirname(__FILE__), "output", "Hshape.osm"))
      osm_model_path = File.absolute_path("../output/Hshape.osm")
      ...
      BTAP::FileIO::save_osm(model, File.join(File.dirname(__FILE__), "output", "Tshape.osm"))
      osm_model_path = File.absolute_path("../output/Tshape.osm")
      ...
      BTAP::FileIO::save_osm(model, File.join(File.dirname(__FILE__), "output", "Ushape.osm"))
      osm_model_path = File.absolute_path("../output/Ushape.osm")
...
#    model.getPlanarSurfaceGroups().each {|planar_surface| planar_surface.changeTransformation(t)}
    ...
    space_type_upgrade_map = standard.standards_lookup_table_many(table_name: 'space_type_upgrade_map')
