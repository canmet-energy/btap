module BTAPMeasureHelper
  ...
  def remove_extra_cooling_eqp(model,zone)

    zoneHVACEquipmentLists=model.getZoneHVACEquipmentLists

      zoneHVACEquipmentLists.each do |eqp_list|
        if eqp_list.thermalZone == zone
          eqp_list.equipment.each do |eqp|
            if eqp.to_ZoneHVACPackagedTerminalHeatPump.is_initialized or eqp.to_ZoneHVACPackagedTerminalAirConditioner.is_initialized or eqp.to_ZoneHVACDehumidifierDX.is_initialized 

              eqp_list.removeEquipment(eqp.to_ModelObject.get) #remove the equipment from the list
              eqp.to_ModelObject.get.remove #remove from model
            end
          end # eqp_list.equipment.each do |eqp|
        end #if eqp_list.thermalZone == zone
      end #zoneHVACEquipmentLists.each do |eqp_list|
  



  end #remove_extra_cooling_eqp(model,zones)


  def remove_extra_heating_eqp(model,zone)
    ...
    zoneHVACEquipmentLists=model.getZoneHVACEquipmentLists
      ...
      zoneHVACEquipmentLists.each do |eqp_list|
        if eqp_list.thermalZone == zone
          eqp_list.equipment.each do |eqp|
            if eqp.to_ZoneHVACBaseboardConvectiveElectric.is_initialized or eqp.to_ZoneHVACBaseboardConvectiveWater.is_initialized or eqp.to_ZoneHVACPackagedTerminalHeatPump.is_initialized 
  ...
  def remove_extra_comp_AddDOASSysAndVAV(model,zone)
    #remove unused 
    if model.building.get.name.to_s.include?("Edmonton") or model.building.get.name.to_s.include?("McMurray") or model.building.get.name.to_s.include?("Yellowknife") #or model.building.get.name.to_s.include?("Highrise")
      puts "Only remove extra cooling equipment"
      remove_extra_cooling_eqp(model,zone)
    else
      puts "Remove extra heating and cooling equipment"
      remove_extra_cooling_eqp(model,zone)
      remove_extra_heating_eqp(model,zone)
    end

    #remove extra erv in the old air loops that are not fan coil units
    model.getAirLoopHVACs.each do |airloop|
      airloop.oaComponents.each do |oacomponent|
        if oacomponent.to_HeatExchangerAirToAirSensibleAndLatent.is_initialized
          old_erv = oacomponent.to_HeatExchangerAirToAirSensibleAndLatent.get
          old_erv.to_ModelObject.get#class
 
          #not removing it - will not affect results bc it's never used (OA system oa flow = 0)
        end
      end
    end
  end # remove_extra_comp_AddDOASSysAndVAV(model,zone)
    
    
  def autosize_affected_hvac_AddDOASSysAndVAV(model,zone)
  ...
  def remove_extra_cooling_eqp_doas_vrf(model,zones)
    ...
    end # zones.each do |zone|
  end #remove_extra_cooling_eqp(model,zones)


  def remove_extra_heating_eqp_doas_vrf(model,zones)
  ...
  def remove_extra_comp_doas_vrf(model,zones)
    #remove unused 
    if model.building.get.name.to_s.include?("Edmonton") or model.building.get.name.to_s.include?("McMurray") or model.building.get.name.to_s.include?("Yellowknife")
      puts "Only remove extra cooling equipment"
      remove_extra_cooling_eqp_doas_vrf(model,zones)
    else
      puts "Remove extra heating and cooling equipment"
      remove_extra_cooling_eqp_doas_vrf(model,zones)
      remove_extra_heating_eqp_doas_vrf(model,zones)

    end

  end #def remove_extra_comp_doas_vrf(model,list_of_vrf_zones)


  def autosize_affected_hvac_doas_vrf(model,list_of_vrf_zones)
    model.getChillerElectricEIRs.each do |chiller|
      if chiller.referenceCapacity.is_initialized 
        cap = chiller.referenceCapacity.get
        if cap > 100 #sometimes chillers are sized to 0.001 in archetypes
  ...
  end # def autosize_affected_hvac_doas_vrf(model,list_of_vrf_zones)
