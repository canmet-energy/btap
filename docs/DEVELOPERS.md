...
`verification/scenarios/` contains 43 scenarios in three lanes:
...
the final attestation identity. The other twelve are Python-only from their first
...
the two DF-17 hydronic-VAV scenarios. A scenario authored after the handoff must
