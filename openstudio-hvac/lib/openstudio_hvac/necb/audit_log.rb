...
module OpenStudioHVAC
  module NECB
    # The decision/audit trail for proposed characterization and reference generation.
    #
    # Every consequential step records WHAT was decided, the INPUTS it was decided from,
    # the model EVIDENCE behind it, and (where applicable) the NECB ARTICLE that mandates
    # it — so QAQC can answer "why did zone X get System 6?" from the log instead of
    # diffing models, and debugging failures reads as a narrative.
    #
    ...
    class AuditLog
      ...
      def decision(step, action, target: nil, inputs: nil, value: nil, article: nil, evidence: nil)
        add(:decision, step, action, target, inputs, value, article, evidence)
      end

      def info(step, action, target: nil, inputs: nil, value: nil, article: nil, evidence: nil)
        add(:info, step, action, target, inputs, value, article, evidence)
      end

      def warn(step, action, target: nil, inputs: nil, value: nil, article: nil, evidence: nil)
        add(:warning, step, action, target, inputs, value, article, evidence)
      end
