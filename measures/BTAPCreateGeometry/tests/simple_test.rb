...
class BTAPCreateGeometry_Test < Minitest::Test
  ...
  def test_sample()
    ...
    assert_in_epsilon(model.getBuilding.floorArea.to_f.round(1), @good_input_arguments['total_floor_area'].to_f.round(1), 0.001, 'floor area')
    assert_equal(model.getBuilding.name.to_s, @good_input_arguments['building_name'].to_s, 'building name')
    assert_includes(model.getBuilding.standardsBuildingType.to_s, @good_input_arguments['template'].to_s, 'code version')
    assert_includes(model.getBuilding.standardsBuildingType.to_s, @good_input_arguments['building_type'].to_s, 'building type')
  end
end
