...
class BtapSetEnvelopeConductanceByNecbClimateZone < OpenStudio::Ruleset::ModelUserScript
 ...
 # values for option 1
  ...
  def run(model, runner, user_arguments)
    ...
    puts "-----------------------Weather File necb hdd--------#{necb_hdd18}-----------------------------------------------------"
	...
	# Check the selected surfact type
	 if surface_type == "Roofs"
      puts ("The selected surface type is '#{surface_type}' So the conductance of roofs only will be changed.")
...
# nothing will be done if there are no exterior surfaces
    ...
    final_constructions_array.each do |final_construction|
      ...
      # test if the roof conductance has changed
       puts "\e[32m ===================== final_conductance_ip is : #{final_conductance_ip} ========================== \e[0m"
    ...
    return true
      ...
      puts ("The selected surface type is '#{surface_type}' So the conductance of walls only will be changed.")
    ...
    return true
      ...
      puts ("The selected surface type is '#{surface_type}' So only the conductance of windows will be changed.")
	...
	#else
	 #puts ("There was no surface selected, Please select a surfece type.")
