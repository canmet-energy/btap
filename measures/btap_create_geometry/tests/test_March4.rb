          ...
          @templates.each do |template|
            @total_floor_area.each do |total_floor_a|
              @above_grade_floors.each do |above_grade_floor|
                ...
                #test_dir = "#{File.dirname(__FILE__)}../output_tests"
...
=end
                ...
                puts ">>>>>>>>>>>>>>>#{template}>template>>>>>>>>>#{input_arguments['template']}"
                ...
                puts">>#{building_type} >>>>>#{building_shape}>>>>model.getBuilding.floorArea >>>#{model.getBuilding.floorArea.to_f.round(3)}, was set to be #{total_floor_a.to_f.round(3)}"
                file.puts "#{building_type} , #{building_shape} , #{template}, #{total_floor_a.to_f.round(3)} , #{model.getBuilding.floorArea.to_f.round(3)}\n \n"
                puts "Building type: #{building_type} , Building shape:  #{building_shape} , Model floor area was set to : #{total_floor_a.to_f.round(3)} , Reported floor area : #{model.getBuilding.floorArea.to_f.round(3)}\n \n"
                ...
                #FileUtils.rm_rf(Dir.glob('SR0'))
                #FileUtils.rm_rf(Dir.glob('SR1'))
                #FileUtils.rm_rf(Dir.glob('SR2'))
                #Dir.rmdir('SR0')
                #Dir.rmdir('SR1')
                #Dir.rmdir('SR2')
