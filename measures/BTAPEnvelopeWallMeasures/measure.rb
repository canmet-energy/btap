...
class IncreaseInsulationRValueForExteriorWalls < OpenStudio::Ruleset::ModelUserScript
  ...
  def look_up_measure_data(csv_fname, search_criteria)
    ...
    raise("More than one match for Wall measure package!") unless matches.size == 1
    return matches[0]
  end
  ...
  def name
    return "BTAP Envelope Wall measure packages"
  end
  ...
  def arguments(model)
    ...
    #    properties = OpenStudio::Ruleset::OSArgument::makeIntegerArgument("",true)
    #    properties.setDisplayName("Package Name")
    #    properties.setDefaultValue("BTAP Wall Package 1")
    #    args << properties
    return args
  ...
  def run(model, runner, user_arguments)
    ...
    package_id = runner.getDoubleArgumentValue("package_id",user_arguments)
    
    measure_data_file = "#{File.dirname(__FILE__)}/resources/measure_data.csv"
    #this will get the corrosponding upgrade package row data based on weather file. 
    row =  look_up_measure_data( measure_data_file, { 'package_id' => 1, 'weather_file'=>model.getWeatherFile.path.get.to_s } )

    row['package_id']
    row['weather_file']
    row['package_name']	
    row['change_conductance_value_method']
    row['labour_cost_per_m2']	
    row['material_cost_per_m2']
    ...
    if r_value < 0 or r_value > 500 #rsi 0.4 to 20
    ...
    surfaces = BTAP::Geometry::Surfaces::filter_by_surface_types(outdoor_surfaces, "Wall")
